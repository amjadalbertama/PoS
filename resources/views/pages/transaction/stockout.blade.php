                ...
                <div class="card">
                        <div class="card-body"><br>
                            ...
                            <div class="row align-items-center justify-content-between mb-3">
                                ...
                                <a class="btn btn-primary tambah-btn" href="{{ url('/add/stockout') }}">Add Stock Out</a>
                                </div>
                            </div>
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        @foreach ($stockOutPag as $stockin)
                                        <tr>
                                            <td>{{ $stockin['id'] }}</td>
                                            <td>id12423</td>
                                            <td>{{ $stockin['code'] }}</td>
                            ...
                            <div class="d-flex justify-content-center mt-3">
                                <nav aria-label="Page navigation">
                                    <ul class="pagination">
                                        ...
                                        @for ($i = 1; $i <= ceil(count($dataStockOut) / $perPage); $i++)
                                        <li class="page-item {{ $i === $currentPage ? 'active' : '' }}">
                                            <a class="page-link" href="{{ $request->url() }}?page={{ $i }}">{{ $i }}</a>
                                        </li>
                                        @endfor

                                        @if ($currentPage < ceil(count($dataStockOut) / $perPage))
                                        <li class="page-item">
